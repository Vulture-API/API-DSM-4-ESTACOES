...
try {
  ...
  const macMisturado = await chamar("POST", "/api/stations", {
    ...base,
    mac_address: mac(9).replace(/:/g, (m, i) => (i % 2 ? "-" : ":")),
  });
  verificar(
    "MAC com separadores misturados devolve 400",
    macMisturado.status === 400,
    macMisturado.status,
  );
  ...
  const semCampos = await chamar("POST", "/api/stations", {
    name: "Só o nome",
  });
  verificar(
  ...
  const filtro = await chamar(
    "GET",
    `/api/stations?property_id=${PROPERTY_ID}`,
  );
  verificar(
  ...
  // Roda mesmo se uma chamada acima lançar (ex.: servidor caiu no meio): o banco
  // de development é compartilhado e não pode ficar com estação de teste.
  console.log("\nExclusão");
  for (const idCriado of criados) {
    const excluido = await chamar("DELETE", `/api/stations/${idCriado}`).catch(
